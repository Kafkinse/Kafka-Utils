...
### Если VPS за NAT (проброс портов)

Релей слушает **внутренний** порт (например 8765), а игроки подключаются к
**публичному** IP и **публичному** порту из проброса. Пример реальной схемы:

| Public IP | Public Port | Inner IP | Inner Port |
|---|---|---|---|
| 163.123.180.81 | 10041 | 192.168.122.28 | 8765 |

Запуск на VPS: `java KafkaRelay.java 8765 КЛЮЧ` (внутренний порт).
URL в моде у игроков: `ws://163.123.180.81:10041` (публичные IP и порт) —
это значение уже стоит по умолчанию в настройке Server URL.
